...
describe("GET /users", function () {
  ...
  //tests BUG#3
  test("should not accept token through query", async function () {
    const response = await request(app).get(`/users?_token=${tokens.u1}`);
    expect(response.statusCode).toBe(401);
  });
});
...
describe("PATCH /users/[username]", function () {
  ...
  // tests BUG#1
  test("should return invalid data error", async () => {
    const response = await request(app).patch("/users/u1").send({
      _token: tokens.u3,
      first_name: "new-fn1",
      invalid_field: "somedata",
    });
